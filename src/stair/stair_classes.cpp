...
Plane Stair::getBestStep() {
    ...
    float sum_of_rectangle_areas, extent;
    for (size_t Q = 1; Q < vLevels.size(); Q++) {
...
void Stair::getExactStepVertices() {
    ...
    if (vLevels[0].cloud->points.size() > 0) {
        ...
        // Check if there are points (at least 5, for example) in around 0.3m from initial point of the stair
        if (neighbour_search(vLevels[0].cloud, initial_point, 0.3f, neighbouring_cloud) > 5) {
...
bool Stair::validateStaircase() {
    ...
    // uncomment for no filter for stair dimensions
    // return valid;

    // Error messages 
    std::string l_error_msg = "LENGTH " + std::to_string(abs(step_length)) + " NOT IN RANGE OF [" + std::to_string(k_length_min) + ", " + std::to_string(k_length_max) + "].";
    std::string w_error_msg = "WIDTH " + std::to_string(abs(step_width)) + " NOT IN RANGE OF [" + std::to_string(k_width_min) + ", " + std::to_string(k_width_max) + "].";
    std::string h_error_msg = "HEIGHT " + std::to_string(abs(step_height)) + " NOT IN RANGE OF [" + std::to_string(k_height_min) + ", " + std::to_string(k_height_max) + "].";

    // Invalid length
    if (abs(step_length) < k_length_min || abs(step_length) > k_length_max) {
    ...
    */
    // Invalid width
    if (abs(step_width) < k_width_min || abs(step_width) > k_width_max) {
        std::cout << w_error_msg << std::endl;
        return false;
    }
    
    // Invalid height
    if (abs(step_height) < k_height_min || abs(step_height) > k_height_max) {
        std::cout << h_error_msg << std::endl;
        return false;
    }
