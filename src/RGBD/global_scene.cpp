...
void GlobalScene::findFloor(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud) {
    ...
    const int k_min_cluster_size = 80;
    
    const float k_max_z_dist = 5.0;
    const float k_z_dist_increase = 0.05f;

    pcl::PassThrough<pcl::PointXYZ> pass;

    while (cloud_filtered->points.size() < k_min_cluster_size && z_dist <= k_max_z_dist) {
...
bool GlobalScene::subtractInitialPlane(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, Eigen::Vector4f &normal) {
    ...
    const int k_min_plane_size = 100;
    const float k_min_percentage_size = 0.20f;
    ...
    *cloud_filtered = *cloud;
    ...
    if (cloud_filtered->points.size() > k_min_plane_size) {  // Three points at least to find a plane
        ...
        seg.setDistanceThreshold(0.005f);  // aprox. Voxel size / 2
        ...
        if (inliers->indices.size() > k_min_plane_size ||
            ...
            if (D < 0) {
                ...
                C = -C;
                D = -D;
            }  // To get the plane normal looking towards the origin
            ...
            const float k_angle_threshold = 10;  // Valid threshold around estimated floor vector
            const float k_min_D = 0.3f;          // Minimum D value (i.e. minimum distance of the floor to the camera allowed)
            const float k_max_D = 1.4f;          // Maximum D value (i.e. maximum distance of the floor to the camera allowed)
...
void GlobalScene::getManhattanDirections(CurrentScene &scene) {
    ...
    if (new_floor_ || initial_floor_) {                        // In findFloorFast has found a new floor, or has been previously computed (and assumed to be more or less constant), perform computation assuming known Y
        ...
        if (scene.has_manhattan_ == false)  // If that didn't work, use normals
            scene.getManhattanDirectionsFromNormalsWithFloor(f2c, c2f);
